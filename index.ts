...
const metricButton = document.querySelector('#metric') as HTMLInputElement;
const imperialButton = document.querySelector('#imperial') as HTMLInputElement;
const imperialDisplay = document.querySelector('#imperial-display') as HTMLDivElement;
const metricDisplay = document.querySelector('#data-input-section') as HTMLDivElement;
const dataCollectionDisplay = document.querySelector('#data-collection-section') as HTMLDivElement;
const metricHeight = document.querySelector('#height') as HTMLInputElement;
const metricWeight = document.querySelector('#weight') as HTMLInputElement;
const bmiDisplay = document.querySelector('#value') as HTMLHeadingElement;
const initialized = document.querySelector('.initialized') as HTMLDivElement;
const welcome = document.querySelector('.welcome') as HTMLDivElement;
const initializedHealthStatus = document.querySelector('#health-status') as HTMLSpanElement;
const initializedWeightRange = document.querySelector('#weight-gauge') as HTMLSpanElement;
const imperialHeightFeet = document.querySelector('#imperial-height') as HTMLInputElement;
const imperialHeightInch = document.querySelector('#imperial-height-inch') as HTMLInputElement;
const imperialWeightStone = document.querySelector('#imperial-weight') as HTMLInputElement;
const imperialWeightPounds = document.querySelector('#imperial-weight-ibs') as HTMLInputElement;
...
imperialHeightFeet.addEventListener('keypress', (event) => { getImperialHeightWeight(event, {
...
imperialHeightInch.addEventListener('keypress', (event) => { getImperialHeightWeight(event, {
...
imperialWeightStone.addEventListener('keypress', (event) => { getImperialHeightWeight(event, {
    firstInputElement: imperialHeightFeet,
...
imperialWeightPounds.addEventListener('keypress', (event) => { getImperialHeightWeight(event, {
    firstInputElement: imperialHeightFeet,
    secondInputElement: imperialHeightInch,
    lastInputElement: imperialWeightStone
}) });

// defining custom type for the object to be passed as argument to the unitDisplayToggle
type displayChanger = {
    firstDisplay: HTMLDivElement,
    firstDisplayOption: string,
    secondDisplay: HTMLDivElement,
    secondDisplayOption: string,
    mainContainer?: HTMLDivElement,
    newHeight?: string
}

// Declaring a function that toggles the display of some elements based on the settings provided to its argument
function unitDisplayToggle(displaySettings: displayChanger): void{
    /**
...
function getMetricUnitBMI(height: number, weight: number): number{
    /**
...
// metric unit. 
function metricHeightWeightcallback(event: KeyboardEvent, measurementType: HTMLInputElement): void {
    /**
    ...
    if (event.key === 'Enter' && event.code === 'Enter'){
        ...
        let result: [string, number, number] = getMetricResultDisplayMessage(Number(metricHeight.value), Number(metricWeight.value));
...
function getMetricResultDisplayMessage(height: number, weight: number): [string, number, number]{
...
function getImperialUnitBMI(heightInFeet: number, heightInInches: number, weightInStone: number, weightInPounds: number): number {
    /**
...
type measurementObject = {
    firstInputElement: HTMLInputElement,
    secondInputElement: HTMLInputElement,
    lastInputElement: HTMLInputElement
}

// Declaring a callback funtion that calls the getImperialUnit BMI function and displays the appropriate message
function getImperialHeightWeight(event: KeyboardEvent, measurementType: measurementObject): void{
    /**
    ...
    if (event.code === 'Enter' && event.key === 'Enter'){
        ...
        const pattern = /^[0-9]+$/;
        if (event.target){
            if ((<HTMLInputElement>event.target).id === "imperial-height"){
            ...
            else if ((<HTMLInputElement>event.target).id === "imperial-height-inch"){
                if ((!pattern.test(imperialHeightFeet.value) || !pattern.test(imperialHeightInch.value)) || (!pattern.test(imperialWeightStone.value) || !pattern.test(imperialWeightPounds.value))){
                    alert('Enter a numeric value');
                    return
                }
            }
            else if ((<HTMLInputElement>event.target).id === "imperial-weight"){
                if ((!pattern.test(imperialHeightFeet.value) || !pattern.test(imperialHeightInch.value)) || (!pattern.test(imperialWeightStone.value) || !pattern.test(imperialWeightPounds.value))){
                    alert('Enter a numeric value');
                    return
                }
            }
            else if ((<HTMLInputElement>event.target).id === "imperial-weight-ibs"){
                if ((!pattern.test(imperialHeightFeet.value) || !pattern.test(imperialHeightInch.value)) || (!pattern.test(imperialWeightStone.value) || !pattern.test(imperialWeightPounds.value))){
                    alert('Enter a numeric value');
                    return
                }
        ...
        const heightFeet: number = Number(imperialHeightFeet.value);
        const heightInch: number =  Number(imperialHeightInch.value);
        const weightStone: number = Number(imperialWeightStone.value);
        const weightPound: number = Number(imperialWeightPounds.value);
...
function getImperialResultMessage(heightFeet: number, heightInch: number, weightStone: number, weightPounds: number): [string, number, number] {
    /**
    ...
    let healthStats: string;
    let miniWeight: number;
    let maxiWeight: number;
